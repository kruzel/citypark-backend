    ...
    function star(object, starsCount, input, callback, readOnly, average) {
        ...
        for (var x = 1; x <= starsCount; x++) {
            stars[x] =
				$("<img></img>")
					.data("index", x)
					.attr("src", average && x <= average ? "/sites/cityp/content/images/staron.png" : "/sites/cityp/content/images/staroff.png")
					.hover(function () {
					    if (!readOnly) {
        ...
        function on(star) {
            star.attr("src", "/sites/cityp/content/images/staron.png");
            return star;
        }

        function off(star) {
            star.attr("src", "/sites/cityp/content/images/staroff.png");
            return star;
        }
    }
...
function AjaxUpload(objectName, type, typeAllowed, folder,preval) {
    window.open('upload.asp?preval='+preval+'&typeAllowed=' + typeAllowed + '&object=' + objectName + '&folder=' + folder + '&type=' + type, "fileUploadWindow", "status = 1, height = 300, width = 300, resizable = 0");
}

function SetFile(objectName, filePath, fileType,preval) {
    ...
    jQuery("#v" + objectName+",#" + objectName).val(preval+filePath);
