...
$(window).load(function() {
			...
			$("#searchaction,#searchaction2").click(function() { //trigger for clicking the submit button
				initPark();
				if (ajaxvar)ajaxvar.abort();
			if ( $("#searchtext").val().split(",").length > 1) { 
					var tempvar = $("#searchtext").val().split(",")[0];
					var cityvar = $("#searchtext").val().split(",")[1].replace(/^\s+|\s+$/g,"");
					var housenum = tempvar.replace(/[^0-9]+/g, "").replace(/^\s+|\s+$/g,"");//var housenum = (str.length>1)?(isnum(str[1])?str[1]:0):0;
					var streetvar = tempvar.replace(/[0-9]+/g, "").replace(/^\s+|\s+$/g,"");//var street = (str.length>0)?str[0]:$("#searchtext").val();
				}
			else 
				{ 
					var tempvar = $("#searchtext").val();
					var cityvar = '';
					var housenum = tempvar.replace(/[^0-9]+/g, "").replace(/^\s+|\s+$/g,"");//var housenum = (str.length>1)?(isnum(str[1])?str[1]:0):0;
					var streetvar = tempvar.replace(/[0-9]+/g, "").replace(/^\s+|\s+$/g,"");//var street = (str.length>0)?str[0]:$("#searchtext").val();
				}
			...
			//	var dpayment = $("#panel input:checkbox[name=free]").is(":checked");
				...
				var dhourly = $("#panel input:checkbox[name=hourly]").is(":checked");
				var dDaily = $("#panel input:checkbox[name=Daily]").is(":checked");
				var dweekly = $("#panel input:checkbox[name=weekly]").is(":checked");
				var dmonthly = $("#panel input:checkbox[name=monthly]").is(":checked");
				var dyearly = $("#panel input:checkbox[name=yearly]").is(":checked");
				...
				var dstartdate = ""; 
				var denddate = "";
				var dpayment = $("select[name=type] option:selected").val().replace("type","");
				var dprice = $("select[name=price] option:selected").val();
				dpayment = dpayment-1;
				if ($("#start_date").val()!='' && $("#end_date").val() !='') {
					...
					ajaxvar = $.ajax({
						type: 'POST',
						url: "parkcore.asp",
						data: {price:dprice,yearly:dyearly,monthly:dmonthly,weekly:dweekly,Daily:dDaily,hourly:dhourly,payment:dpayment,house:housenum,startdate:dstartdate,enddate:denddate,distance:ddistance,street:streetvar,city:cityvar,payment:dpayment,limit:dlimit,barrier:dbarrier,underground:dunderground,walls:dwalls,disabled:ddisabled,m:"srch"},
		...
		switch(e.keyCode) {
			...
			default:
				...
				if ($("#searchtext").val().length > 1) {
						if (ajaxvar)ajaxvar.abort();
						var str = $("#searchtext").val();//var str = $("#searchtext").val().split(",")[0].replace(/,/g," ").replace(/  /g," ").split(" ");
						var housenum = str.replace(/[^0-9]+/g, "");//var housenum = (str.length>1)?(isnum(str[1])?str[1]:0):0;
						var city = str.replace(/[0-9]+/g, "").split(",")[1];//var city = $("#searchtext").val().split(",")[1];
						var street = str.replace(/[0-9]+/g, "").split(",")[0];//var street = (str.length>0)?str[0]:$("#searchtext").val();
						
						ajaxvar = $.ajax({
							type: 'POST',
							url: "parkcore.asp",
							// data: {m:"ac",search:$("#searchtext").val()},
							data: {m:"ac",street:street,housenum:housenum,city:city,search:street},
							success: function(data) {
...
function clc() {
$('.valu').html(' עלות שעת חניה '+parseFloat($('#fancybox-inner .calccity option:selected').val()).toFixed(1)+'  שקלים <br />מחיר מומלץ לשעה בחניה פרטית '+parseFloat( $('#fancybox-inner .calccity option:selected').val() * $('#fancybox-inner .yemama option:selected').val()).toFixed(1)+'  שקלים');
}