...
<% 
End If
CheckSecuirty "BlogPost"

If  Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
		SQL = "SELECT * FROM BlogPost WHERE SiteID=" & SiteID & " ORDER BY BlogPostID DESC"
	If Request.QueryString("mode") = "search" then
		SQL = "SELECT * FROM BlogPost WHERE SiteID=" & SiteID & " AND BlogPostName LIKE '%" & Request.form("search") & "%'" & "  AND Active=" & Int(Request.form("Active"))& " ORDER BY BlogPostID DESC"
	End If
...
<center>
    <div style="width:99%;height:43px;background:#fff;line-height:43px;border:1px solid #ccc;">
        ...
        <div style="width:40px;float:left;line-height:43px;"><a href="admin_BlogPost.asp?action=add"><img src="../admin/images/add.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></div>
        <div style="width:40px;float:left;line-height:43px;"><a href="admin_BlogPost.asp?format=xls&<%Request.Querystring %>"><img src="../admin/images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></div>
        <div style="width:40px;float:left;line-height:43px;"><a href="admin_BlogPost.asp?format=doc&<%Request.Querystring %>"><img src="../admin/images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></div>
        <div style="float:left;line-height:43px;margin-left:10px;width:50px;">
        <select style="float:right;font-family:arial;height:23px;margin-top:9px;width:50px;" name="records" onchange="location.href='admin_BlogPost.asp?records='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
        ...
        <form action="admin_BlogPost.asp?action=show&mode=search&records=<%=Request.Querystring("records") %>" method="post">
		            ...
		            <td style="border-bottom:1px solid #ccc;height:25px;vertical-align:middle;" align="center" colspan="8">
	      	            <table width="200" cellspacing="0" align="center">
	                        ...
	                        <% If objRs.AbsolutePage > 1 Then %>
					                        <a href="admin_BlogPost.asp?page=<% = objRs.AbsolutePage - 1 %>" style="text-decoration: none"><% End If%>
...
<tbody>
    ...
    <tr onmouseover="Mark(this);" onmouseout="Unmark(this);">
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"><% = objRs("BlogPostID") %></td>
        <td style="padding-right:10px;border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="right"><% = objRs("BlogPostName") %></td>
        <%if format = "" then %>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_BlogPost.asp?ID=<% = objRs("BlogPostID") %>&action=edit"><img src="/sites/cms/layout/images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_BlogPost.asp?ID=<% = objRs("BlogPostID") %>&action=copy"><img src="/sites/cms/layout/images/copy.gif" border="0" alt="שכפל דף" /></a></td>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_Menu.asp?action=add&src=<% =objRs("BlogPostID") %>&Headline=<% =objRs("BlogPostName") %>&lang=<% = SiteLang %>&S=<% = SiteID %>"><img src="/sites/cms/layout/images/link.gif" border="0" alt="צור קישור" /></a></td>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center">
        
        &nbsp;</td>
        <td style="border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_BlogPost.asp?ID=<% = objRs("BlogPostID") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק דף זה');"><img src="/sites/cms/layout/images/delete.gif" border="0" alt="מחק" /></a></td>
        <% End If ' format %>
    </tr>	
    			<% HowMany = HowMany + 1
				objRs.MoveNext
				            ...
				            %>
					            <tr>
							            ...
							            <td width="177"><font size="2" face="Arial">
							                <div align="center">
	                        <% If objRs.AbsolutePage > 1 Then %>
					                        <a href="admin_BlogPost.asp?page=<% = objRs.AbsolutePage - 1 %>&records=<% = records %>" style="text-decoration: none"><% End If%>
					                        <font color="#808080">דף קודם</font></a>
					                    </td>
					                        <td width="25">
...
Else
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
	        Case "add"
	        Editmode= "False"
			      Set objRsUrl = OpenDB("SELECT * FROM BlogPost WHERE SiteID=" & SiteID)

			        Sql = "SELECT * FROM BlogPost WHERE SiteID= " & SiteID
			        	Set objRs = OpenDB(sql)
	                    ...
	                    CheckCategorySecuirty(Request.Form("CategoryID"))
				             ...
				             objRs.Addnew
				             objRs("SiteID") = SiteID
				             objRs("BlogPostName") = Request.Form("BlogPostName")
								objRs("BlogPostDescription") = Request.Form("BlogPostDescription")	
								objRs("Image") = Trim(Request.Form("Image"))
								'objRs("BlogPostUsersID") = Int(Request.Form("BlogPostUsersID"))	
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>בלוג נוסף בהצלחה. <a href='admin_BlogPost.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
					...
					If GetConfig("twitterusername") <> "" AND Request.Form("sendtotwitter") = 1 Then
                        ...
                        new_status = Trim(Request.Form("BlogPostName")& " <br> -" & url )  'change to your new status  
                        ...
                        xml.Send  
						Response.Write("<br><br><p align='center'>סטטוס עודכן בטוויטר. <a href='admin_BlogPost.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
                        Set xml = Nothing  
                     End If
							     '   Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_BlogPost.asp?S=" & SiteID & "'>")
	       ...
	       Case "edit"
	       	 Editmode= "True"
				    ...
				    Sql = "SELECT * FROM BlogPost WHERE BlogPostID=" & Request.QueryString("ID") & " AND SiteID= " & SiteID
			            Set objRs = OpenDB(sql)
				             ...
				             objRs("BlogPostName") = Request.Form("BlogPostName")
								...
								objRs("ViewLevel") = Int(Request.Form("ViewLevel"))	
								objRs("PostCommentLevel") = Int(Request.Form("PostCommentLevel"))
								'objRs("BlogPostDate") = Request.Form("BlogPostDate")
								    If Request.Form("Active")<> 1 Then
								        objRs("Active") = 0
								    Else
								        objRs("Active") = 1
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>בלוג נערך בהצלחה. <a href='admin_BlogPost.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_BlogPost.asp?S=" & SiteID & "'>")
	        ...
	        Case "copy"
	        	Editmode = "True"
				    ...
				    Sql = "SELECT * FROM BlogPost WHERE BlogPostID=" & Request.QueryString("ID")
			        	Set objRs = OpenDB(sql)
	        ...
	        Case "delete"
				    Sql = "SELECT * FROM BlogPost WHERE BlogPostID=" & Request.QueryString("ID")
			            ...
			            objRs.Delete
			            objRs.Close
						    Response.Write("<br><br><p align='center'>בלוג נמחק בהצלחה. <a href='admin_BlogPost.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							Response.Write("<meta http-equiv='Refresh' content='0; URL=admin_BlogPost.asp?S=" & SiteID & "'>")
		...
		Else
			...
			If Request.QueryString("action") = "add" Then
			 SQL = "SELECT * FROM BlogPost WHERE BlogPostID=" & SiteID   
			End If	
	Set objRs = OpenDB(SQL)
 ...
 %>
<form action="admin_BlogPost.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("BlogPostID")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2" class="_validate">
        ...
        <div id="formrightfields">
            <table dir="rtl" cellpadding="5" cellspacing="0" width="99%" style="margin-top:10px;">
                <tr>
                  ...
                  <textarea name="BlogPostName" cols="3" style="width:80%; height: 20px; font-family:Arial;" 
                            class="required"><%If Editmode = "True" Then print objRs("BlogPostName") End If %><% if request.querystring("name")<> "" Then print request.querystring("name") end if%></textarea> </td>
                </tr>
                <tr>
...
If Editmode = "True" Then
    oFCKeditor.Value=objRs("BlogPostDescription")
End If
...
End If
oFCKeditor.Create "BlogPostDescription"
...
%>
               ...
               <% End If %>
                <tr>
                 ...
                 <td align="right"><input id="Checkbox1"  type="checkbox" dir="ltr" name="active" value="1" <%If Editmode = "True" Then %><% If objRs("active")= 1 Then print " checked=""yes""" %><% Else %> checked="yes"<% End If %>" onchange="" maxlength="" value="" style="float:right;" class="" format="" /></td>
               </tr>
