...
CheckSecuirty "tlcmeat"
    if request("inline") = "true" Then
        ...
        ExecuteRS "UPDATE [tlcmeat] Set " & fieldName & " = '" & value & "' WHERE Id = " & rowId
	    Response.End
    end if
 format = Request.QueryString("format")
    If  format  = "xls" Then
        ...
        Response.AddHeader "tlcmeat-Disposition", "attachment;filename=tlcmeat-"&filedate&".xls"
        Response.tlcmeatType = "application/ms-excel" 
     End If
         If  format  = "doc" Then
        Response.Buffer = True
        filedate = day(now())&"-"&month(now())&"-"&year(now())&"_"&Siteid
        Response.AddHeader "tlcmeat-Disposition", "attachment;filename=tlcmeat-"&filedate&".doc"
        Response.tlcmeatType = "application/vnd.ms-word"
     End If
...
CheckSecuirty "tlcmeat"
 %>   
    <div class="formtitle">
        <h1>ניהול מנות - בשר</h1>
		<div class="admintoolber">
		</div>
		<div class="adminicons">
				<p><a href="admin_tlcmeat.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></p>
...
Do While Not objRs.EOF And HowMany < objRs.PageSize
    ...
    <tr>
        ...
        <td><a href="admin_tlcmeat.asp?ID=<% = objRs("Id") %>&action=edit"><img src="images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td><a href="admin_tlcmeat.asp?ID=<% = objRs("Id") %>&action=copy"><img src="images/copy.gif" border="0" alt="שכפל " /></a></td>
        <td style="border-left:0px;"><a href="admin_tlcmeat.asp?ID=<% = objRs("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק ');"><img src="images/delete.gif" border="0" alt="מחק" /></a></td>
	</tr>	
 <%	 HowMany = HowMany + 1
       objRs.MoveNext
		 Loop
...
Else
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
	        Case "add"
	        Editmode= "False"
			      Set objRsUrl = OpenDB("SELECT * FROM tlcmeat WHERE SiteID=" & SiteID)

			        Sql = "SELECT * FROM tlcmeat"
			        	Set objRs = OpenDB(sql)
				             ...
				             objRs("SiteID") = SiteID
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>נוסף בהצלחה. <a href='admin_tlcmeat.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' users='1; URL=admin_tlcmeat.asp?S=" & SiteID & "'>")
	       Case "edit"
	       	 Editmode= "True"

				    Sql = "SELECT * FROM tlcmeat WHERE ID=" & Request.QueryString("ID")
			            Set objRs = OpenDB(sql)
				                objRs("Name") = Trim(Request.Form("Name"))

								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>נערך בהצלחה. <a href='admin_tlcmeat.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' users='1; URL=admin_tlcmeat.asp?S=" & SiteID & "'>")
	        ...
	        Case "delete"
				    Sql = "SELECT * FROM tlcmeat WHERE id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            Set objRs = OpenDB(sql)
			            objRs.Delete
			            objRs.Close
						    Response.Write("<br><br><p align='center'>נמחק בהצלחה. <a href='admin_tlcmeat.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							Response.Write("<meta http-equiv='Refresh' users='0; URL=admin_tlcmeat.asp?S=" & SiteID & "'>")
	            End Select
		Else
				SQL = "SELECT * FROM tlcmeat WHERE ID=" & Request.QueryString("ID") & " And SiteID=" & SiteID
...
<form action="admin_tlcmeat.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="tlcmeat2" name="tlcmeat2" class="_validate">
