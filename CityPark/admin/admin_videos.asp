...
CheckSecuirty "videos"
    if request("inline") = "true" Then
        ...
        ExecuteRS "UPDATE [videos] Set " & fieldName & " = '" & value & "' WHERE Id = " & rowId
	    Response.End
    end if
...
CheckSecuirty "videos"
 %>   
    <div class="formtitle">
        <h1>ניהול סרטוני וידיאו</h1>
		<div class="admintoolber">
        <form action="admin_videos.asp?action=show&mode=search&records=<%=Request.Querystring("records") %>&string=<%=Request.form("search") %>" method="post">
               <p>חיפוש:</p>
			   <select name="Active">
		...
		<div class="adminicons">
			<p><a href="admin_videos.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></p>
			<p><a href="admin_videos.asp?format=xls&<%Request.Querystring %>"><img src="images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p><a href="admin_videos.asp?format=doc&<%Request.Querystring %>"><img src="images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p>
				<select name="records" onchange="location.href='admin_videos.asp?records='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
...
<thead>
    <tr>
		...
		<th style="width:8%;"><a href="javascript:void(0)" onclick="window.open('admin_videos_positions.asp?type=videos&action=edit','welcome','width=600,height=50000')">סדר בנים</a></th>
        <th>ערוך</th>
        <th>שכפל דף</th>
        <th>פעיל</th>
        <th>מחק</th>
...
If Request.QueryString("p")= "" AND Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
           '     SQL = "SELECT [Content].*, Contentfather.FatherID FROM [Content] INNER JOIN Contentfather ON [Content].Id = Contentfather.Contentid WHERE (Contentfather.FatherID = " & id & ") AND Content.SiteID=" & SiteID & " AND (Contenttype = 5)  ORDER By ItemOrder ASC"
                SQL = "SELECT * FROM [Content]  WHERE SiteID=" & SiteID & " AND (Contenttype = 6)  ORDER By ItemOrder ASC"
     If Request.QueryString("mode") = "search" then
            ...
            If Request.form("text") <> "" Then
             ...
             End If
                ...
                SQL = SQL &  " AND (Content.SiteID=" & SiteID & ") AND (Contenttype = 6)  ORDER By ItemOrder ASC"
    End If
    ...
    <tr id="node-<%= objRsg("Id") %>" <% if id <> 0 Then %>class="child-of-node-<%= id %>"<% end if %>>
        ...
        <a href="javascript:void(0)" onclick="window.open('admin_videos_positions.asp?type=content&ID=<% = objRsg("Id") %>&action=edit','welcome','width=600,height=50000')" class="_dialog"><img src="images/order.gif" border="0" alt="סדר בנים" /></a>
        <% End If %>
		-->
        </td>
        <td><a href="admin_videos.asp?ID=<% = objRsg("Id") %>&action=edit"><img src="images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td><a href="admin_videos.asp?ID=<% = objRsg("Id") %>&action=copy"><img src="images/copy.gif" border="0" alt="שכפל דף" /></a></td>
        ...
        <td style="border-left:0px;"><%If objRs2.RecordCount < 1 Then %><a href="admin_videos.asp?ID=<% = objRsg("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק דף זה');"><img src="images/delete.gif" border="0" alt="מחק" /></a> <% End If %></td>
			      ...
			      Set objRsUrl = OpenDB("SELECT * FROM Content WHERE SiteID=" & SiteID)
			        ...
			        Sql = "SELECT * FROM Content WHERE SiteID= " & SiteID
					    ...
					    objRs("Menus") = c
                            ...
                            Response.Redirect("admin_videos.asp?notificate=גלרייה נוספה בהצלחה")
                  End If
	       Case "edit"
	       	 Editmode= "True"
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID= " & SiteID
                        ...
                        ExecuteRS("DELETE FROM [Contentfather] WHERE ContentID = " & Request.QueryString("ID"))
                            ...
                            Response.Redirect("admin_videos.asp?notificate=גלרייה נערכה בהצלחה")
					    ...
					    objRs("Menus") = c
                               ...
                               For Each x In Request.Form
								...
								Next
									...
									Response.Write("<br><br><p align='center'>תוכן נוסף בהצלחה. <a href='admin_videos.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_videos.asp?S=" & SiteID & "'>")
                        End If
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            ...
			            objRs.Close
                         ExecuteRS("DELETE FROM [Contentfather] WHERE ContentID = " & Request.QueryString("ID"))
						    ...
						    Response.Write("<br><br><p align='center'>גלרייה נמחקה בהצלחה. <a href='admin_videos.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							Response.Write("<meta http-equiv='Refresh' content='0; URL=admin_videos.asp?S=" & SiteID & "'>")
...
<form action="admin_videos.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2" class="_validate">
			<div class="incontentboxgrid2">
				<div class="formtitle2">
					<% If  Request.QueryString("action") = "add" Then %>
					<h1>הוספת סרטון וידיאו</h1>
				<% ELSE 
				%>
					<h1>עריכת סרטון וידיאו</h1>
				<% End If %>
				</div>
			    <div class="rightform">
					<table class="tableform" dir="rtl" cellpadding="0" cellspacing="0" width="100%">
					<tr>
						<td width="20%" align="right"><p>שם סרטון (H1):</p><img src="images/ask22.png" id="east" original-title="כותרת הדף כמו שתופיע באתר<br />חשוב לקידום האתר בגוגל" /></td>
					...
					<tr>
						...
						<% End If %>
						 <tr>
					  <td align="right" valign="top"><div style="float:right;height:auto;padding:4px 0 0;width:100%;"><p>קטגוריה/גלרייה:</p><img src="images/ask22.png" id="east3" original-title="ניתן לבחור יותר מקטגוריה אחת לדף" /></div></td>
					  <td align="right">
                 ...
                 <tr>
					...
					<td width="80%" align="right">
					<input class="goodinputshort" id="Image" name="Image" dir="ltr" <% If Editmode = "True" Then %>  value="<% =objRs("Image")%>" <% End If %>type="text" size="60" />
					<input class="goodinputbrows" type="button" value="חפש בשרת" onclick="BrowseServer('Image');"/><font size="2">
					</td>			      
			    </tr>
			    <tr>
				    <td width="20%" align="right">סרטון:</td>				
					<td width="80%" align="right">
					<input class="goodinputshort" id="other1" name="other1" dir="ltr" <% If Editmode = "True" Then %>  value="<% =objRs("other1")%>" <% End If %>type="text" size="60" />
					<input class="goodinputbrows" type="button" value="חפש בשרת" onclick="BrowseVideo('other1');"/><font size="2">
					</td>			      
			    </tr>
				<tr>
					<td width="20%" align="right">קישור:</td>
					<td width="80%" align="right">
						<input class="goodinputlong" type="text" dir="ltr" name="other2" maxlength="100"<% If Editmode = "True" Then %> value="<% = objRs("other2") %>"<% End If %>><font size="2">
					</td>
			    </tr>
					<tr>
    ...
    <div id="formcontent">
        <div id="formleftfields">
		    <div style="width:100%;" id="accordion">
				...
				<h3><a style="background:url(images/descshort.png) no-repeat right;" href="#">תיאור קצר</a></h3>
			        <div>
