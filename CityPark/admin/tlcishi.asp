...
text2send2 = text2send2 & "<h1 style=""color:#fff;padding:0 10px 0 0;float:right;width:500px;"">דוח תפריטים " & objRs("tlchomesName")  & "</h1>"
...
text2send2 = text2send2 & "<tr style=""background:#ddd;""><th>שם הדייר</th><th>יום ראשון</th><th>יום שני</th><th>יום שלישי</th><th>יום רביעי</th><th>יום חמישי</th></th></b>"
...
Set objRsm=Opendb("select * from tlcmenus where tlchomesID =" & Request.querystring("Id") & " AND tlcclassID=" & objRsmaj("tlcclassID"))
	If objRsm.recordcount > 0 then
		...
		Set objRsu=Opendb("select * from users where Id =" & objRsm("UsersID"))
		If objRsu.recordcount > 0 then
			text2send2 = text2send2 & "<tr><td>" & objRsu("Other7")  & "</td><td>" & objRsm("SunMeat") & " - " & objRsm("SunSoup") & "</td><td>" & objRsm("MonMeat") & " - " & objRsm("MonSoup") & "</td><td>" & objRsm("ThrMeat") & " - " & objRsm("ThrSoup") & "</td><td>" & objRsm("WedMeat")& " - " & objRsm("WedSoup") & "</td><td>"  &objRsm("ThuMeat")& " - "& objRsm("ThuSoup") & "</td></tr>"
		End if
		objRsm.movenext
		loop
	End if
...
Next
	If request.querystring("mode") ="sendmail" then
		sendmail "דוח חלוקת מזון מחלקתי ל: " & objRs("tlchomesName"),"tafrit@tlc.co.il",objRs("ManagerMail"),text2send2 
