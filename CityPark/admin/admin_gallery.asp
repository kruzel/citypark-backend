...
CheckSecuirty "gallery"
    if request("inline") = "true" Then
        ...
        ExecuteRS "UPDATE [gallery] Set " & fieldName & " = '" & value & "' WHERE Id = " & rowId
	    Response.End
    end if
...
CheckSecuirty "gallery"
 %>   
    <div class="formtitle">
        ...
        <form action="admin_gallery.asp?action=show&mode=search&records=<%=Request.Querystring("records") %>&string=<%=Request.form("search") %>" method="post">
               <p>חיפוש:</p>
			   <select name="Active">
		...
		<div class="adminicons">
			<p><a href="admin_gallery.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></p>
			<p><a href="admin_gallery.asp?format=xls&<%Request.Querystring %>"><img src="images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p><a href="admin_gallery.asp?format=doc&<%Request.Querystring %>"><img src="images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p>
				<select name="records" onchange="location.href='admin_gallery.asp?records='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
...
If Request.QueryString("p")= "" AND Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
           '     SQL = "SELECT [Content].*, Contentfather.FatherID FROM [Content] INNER JOIN Contentfather ON [Content].Id = Contentfather.Contentid WHERE (Contentfather.FatherID = " & id & ") AND Content.SiteID=" & SiteID & " AND (Contenttype = 4)  ORDER By ItemOrder ASC"
                SQL = "SELECT * FROM [Content]  WHERE SiteID=" & SiteID & " AND (Contenttype = 4)  ORDER By ItemOrder ASC"
     If Request.QueryString("mode") = "search" then
            ...
            If Request.form("text") <> "" Then
             ...
             End If
                ...
                SQL = SQL &  " AND (Content.SiteID=" & SiteID & ") AND (Contenttype = 4)  ORDER By ItemOrder ASC"
    End If
    ...
    <tr id="node-<%= objRsg("Id") %>" <% if id <> 0 Then %>class="child-of-node-<%= id %>"<% end if %>>
        ...
        <a href="javascript:void(0)" onclick="window.open('admin_gallery_positions.asp?type=content&ID=<% = objRsg("Id") %>&action=edit','welcome','width=600,height=50000')" class="_dialog"><img src="images/order.gif" border="0" alt="סדר בנים" /></a>
        <% End If %>
		-->
        </td>
        <td><a href="admin_gallery.asp?ID=<% = objRsg("Id") %>&action=edit"><img src="images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td><a href="admin_gallery.asp?ID=<% = objRsg("Id") %>&action=copy"><img src="images/copy.gif" border="0" alt="שכפל דף" /></a></td>
        ...
        <td style="border-left:0px;"><%If objRs2.RecordCount < 1 Then %><a href="admin_gallery.asp?ID=<% = objRsg("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק דף זה');"><img src="images/delete.gif" border="0" alt="מחק" /></a> <% End If %></td>
			        ...
			        Sql = "SELECT * FROM Content WHERE SiteID= " & SiteID
			        	Set objRs = OpenDB(sql)
				             ...
				             objRs("Name") = Request.Form("Name")
								...
								objRs("other6") =Request.Form("other6") 'סדר מיון
					    ...
					    objRs("Menus") = c
                            ...
                            Response.Redirect("admin_gallery.asp?notificate=גלרייה נוספה בהצלחה")
                  End If
	       Case "edit"
	       	 Editmode= "True"
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID= " & SiteID
                        ...
                        ExecuteRS("DELETE FROM [Contentfather] WHERE ContentID = " & Request.QueryString("ID"))
                            ...
                            Response.Redirect("admin_gallery.asp?notificate=גלרייה נערכה בהצלחה")
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            ...
			            objRs.Close
                         ExecuteRS("DELETE FROM [Contentfather] WHERE ContentID = " & Request.QueryString("ID"))
						    ...
						    Response.Write("<br><br><p align='center'>גלרייה נמחקה בהצלחה. <a href='admin_gallery.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							Response.Write("<meta http-equiv='Refresh' content='0; URL=admin_gallery.asp?S=" & SiteID & "'>")
...
<form action="admin_gallery.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2" class="_validate">
			<div class="incontentboxgrid2">
				<div class="formtitle2">
					<% If  Request.QueryString("action") = "add" Then %>
					<h1>הוספת גלריית תמונות</h1>
				<% ELSE 
				%>
					<h1>עריכת גלריית תמונות</h1>
				<% End If %>
				</div>
			    <div class="rightform">
                            ...
                            <script type="text/javascript">
							 ...
							 addCategory =  function(id, selected, del) {
								jQuery(function($) {
									...
									Set objRsCategory = OpenDB("SELECT * FROM Content Where id <>" &  objRs("id") & " AND SiteID= " & SiteID & "  ORDER BY id ASC")			
                                    else
									Set objRsCategory = OpenDB("SELECT * FROM Content Where SiteID= " & SiteID & "  ORDER BY id ASC")			
                                    End if
									%>
									var op1 = $('<option />')
										.val('<% = 0 %>')
                    ...
                    <tr>
				    <td width="15%" align="right">ספרייה בשרת:</td>				
					<td width="35%" align="right">
				        <select class="goodselect" name="other1" size="1">

                            <% 
                            set fs=Server.CreateObject("Scripting.FileSystemObject")
                            set fo=fs.GetFolder(phisicalpath & Getconfig("Sitename")& "\content\images\")
                                for each x in fo.SubFolders 
                                %>
                                        <option value="<% = x.name %>" <% If x.name = objRs("other1") Then Response.Write(" selected") %>><% = x.name %></option>
                                <%   next
                            set fo=nothing
                            set fs=nothing %>
					    </select>
					  ...
					  <td width="15%" align="right" valign="top"><p>להציג את כל הספרייה:</p></td>
					  <td width="35%" align="right" valign="top"><input id="other5"  type="checkbox" dir="ltr" name="other5" value="1" <%If Editmode = "True" Then %><% If objRs("other5")= 1 Then print " checked=""yes""" %><% Else %> checked="yes"<% End If %>" onchange="" maxlength="" value="" style="float:right;" class="" format="" />
					</td>
                    </tr>
                    <tr>
					  <td width="15%" align="right" valign="top"><p>סדר מיון:</p></td>
                        <td>
					  <select  class="goodselect validate" name="other6">
					    <option value="ID Desc"<%If Editmode = "True" Then %> <% If objRs("other6") = "ID Desc" Then Response.Write(" selected") %><%End If %>>אחרון שהוכנס מופיע ראשון</option>
					    <option value="ID ASC" <%If Editmode = "True" Then %><% If objRs("other6") = "ID ASC" Then Response.Write(" selected") %><%End If %>>ראשון שהוכנס מופיע ראשון</option>
					    <option value="Itemorder ASC"<%If Editmode = "True" Then %> <% If objRs("other6") = "Itemorder ASC" Then Response.Write(" selected") %><%End If %>>מיון ידני לפי סדר מיון</option>
					    <option value="Itemorder Desc"<%If Editmode = "True" Then %> <% If objRs("other6") = "Itemorder Desc" Then Response.Write(" selected") %><%End If %>>מיון ידני מהאחרון לראשון</option>
					    <option value="NEWID()"<%If Editmode = "True" Then %> <% If objRs("other6") = "NEWID()" Then Response.Write(" selected") %><%End If %>>רנדומאלי</option>
					  </select>				  
					</td>
				  </tr>
