...
<script type="text/javascript">
    jQuery(document).ready(function () {
		...
		$("#categorylist").load("admin_ajax.asp?ajax=4&id=<%=Request.QueryString("id")%>&action=<%=Request.QueryString("action")%>",function() {initTree();});
...
CheckSecuirty "Content"
 %>   
    <div class="formtitle">
        <h1>ניהול טפסים ושאלונים</h1>
		<div class="admintoolber">
        <form action="admin_Customform.asp?action=show&mode=search&records=<%=Request.Querystring("records") %>&category=<%=Request.form("category") %>&string=<%=Request.form("search") %>&lang=<%=Request.form("lang") %>" method="post">
               <p>חיפוש:</p>
			   <select name="Active">
		...
		<div class="adminicons">
			<p><a href="admin_Customform.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת טופס" title="הוספת טופס" /></a></p>
			<p>
			...
			<p class="reshumot">רשומות לדף:</p>
						...
						<select style="width:auto;"  name="LangID" onchange="location.href='admin_Customform.asp?mode=lang&lang='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
							<option value="">הכל</option>
...
<thead>
    <tr>
        ...
        <th style="width:8%;">נהל נתונים</th>
        <th style="width:8%;">ערוך שדות</a></th>
		<th style="width:8%;">ערוך טופס</th>
        <th style="width:8%;">שכפל טופס</th>
        <th style="width:8%;">מחק</th>
        <th style="width:8%;">ערוך תשובות</a></th>
    </tr>
</thead>
<tbody>
<% 
Function bringrecrds(id, color, stay)
If Request.QueryString("p")= "" AND Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
               SQL = "SELECT * FROM [Content] WHERE  Content.SiteID=" & SiteID & " AND  (Contenttype = 7)  AND (LangID LIKE '%" & Request.querystring("lang") & "%') ORDER By LangID ASC,ItemOrder ASC"
   '  print sql
    ...
    <tr id="node-<%= objRsg("Id") %>" <% if id <> 0 Then %>class="child-of-node-<%= id %>"<% end if %> style="background-color: <% if color then %>#fff<% Else %>#f5f5f5<%End If %>;">
		...
		<td class="editlocal"><a href="Admin_Customformdata.asp?formid=<% = objRsg("Id") %>"><img src="images/order.gif" border="0" alt="סדר בנים" /></a></td>
        <td class="editlocal"><a href="admin_Customformfields.asp?formid=<% = objRsg("Id") %>"><img src="images/order.gif" border="0" alt="נהל שדות" /></a></td>
         <td><a href="admin_Customform.asp?ID=<% = objRsg("Id") %>&action=edit"><img src="images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td><a href="admin_Customform.asp?ID=<% = objRsg("Id") %>&action=copy"><img src="images/copy.gif" border="0" alt="שכפל דף" /></a></td>
        <td style="border-left:0px;"><a href="admin_Customform.asp?ID=<% = objRsg("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק דף זה');"><img src="images/delete.gif" border="0" alt="מחק" /></a>
		</td>
        <% if objRsg("other6") = "2" then %>
            <td class="editlocal"><a href="admin_Customformanswers.asp?formid=<% = objRsg("Id") %>"><img src="images/order.gif" border="0" alt="נהל תשובות" /></a></td>
		<% End if %>
    </tr>	
 <%		
...
Else
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
	        Case "add"
	   SqlCategory2 = "SELECT  TOP 1 * FROM Content WHERE SiteID= " & SiteID & " ORDER BY Id DESC"
			        ...
			        Sql = "SELECT * FROM Content WHERE SiteID= " & SiteID
			        	Set objRs = OpenDB(sql)
				             ...
				             objRs("urltext") = Trim(Request.Form("urltext"))
								...
								objRs("other1") = Request.Form("other1") 'כתובת אימייל מנהל
								objRs("other2") = Request.Form("other2") 'שדה בטופס שמייצג את האימייל של הלקוח
								objRs("other3") = Request.Form("other3")'הודעה אחרי שהטופס נשלח
								objRs("other4") = Request.Form("other4")'הפנייה אחרי מילוי
								objRs("other5") = Request.Form("other5")'זמן להפנייה
								objRs("other6") = Request.Form("other6")'טופס או שאלון
         						objRs("other7") = Request.Form("other7")'טקסט כפתור
            ...
            tSqll =objRsSqll(0)
            CloseDB(objRsSqll)
            Response.Redirect("admin_Customformfields.asp?formid="& tSqll &"&action=edit")
							  '      Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_Customform.asp?S=" & SiteID & "'>")
	       ...
	       Case "edit"
	       	 Editmode= "True"
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID= " & SiteID
                        ...
                        xml.Send  
						Response.Redirect("admin_Customform.asp?notificate=הדף נשלח לטוויטר")
                        Set xml = Nothing  
                     End If
					 Response.Redirect("admin_Customform.asp?notificate=הדף נערך בהצלחה")
                    else
                        Response.Redirect("admin_Customform.asp?ID="&Request.QueryString("ID")&"&action=edit")
                    end if
	        Case "copy"
	        	Editmode = "True"
	                    SqlCategory2 = "SELECT  TOP 1 * FROM Content WHERE SiteID= " & SiteID & " AND (Contenttype = 7) ORDER BY Id DESC"
		                Set objRsCategory2 = OpenDB(SqlCategory2)
                                If objRsCategory2.RecordCount = 0 then
                                    LastNews = 0
                                 Else
			    ...
			    End If
					    ...
					    objRs("Menus") = c
								...
								Next
									...
									Response.Redirect("admin_Customform.asp?notificate=הדף נוסף בהצלחה")
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            ...
			            objRs.Close
                         ExecuteRS("DELETE FROM [Contentfather] WHERE ContentID = " & Request.QueryString("ID"))
						    ...
						    Response.Redirect("admin_Customform.asp?notificate=הדף נמחק בהצלחה")
...
<form action="admin_Customform.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2" class="_validate">
...
טוען...
	...
	%>
				   ...
				   <% End If %>
					...
					<tr>
						<td align="left" colspan="2"><input type="submit" name="save" style="margin:10px 0 0;" value="שמור" class="saveform"  />
           <input type="submit" name="update" style="margin:10px 0 0 10px;" value="עדכן" class="saveform"  /></td>
    ...
    <div id="formcontent">
        <div id="formleftfields">
           ...
           <h3><a style="background:url(images/banim.png) no-repeat right;" href="#">הגדרת בנים</a></h3>
                   ...
                   <table class="leftstuff" dir="rtl" cellpadding="3" cellspacing="0" border="0" width="100%" id="Table3">
					...
					<td>
					  <select  class="goodselect validate" name="Sonsorder">
					    ...
					    <option value="Itemorder ASC"<%If Editmode = "True" Then %> <% If objRs("Sonsorder") = "Itemorder ASC" Then Response.Write(" selected") %><%End If %>>מיון ידני לפי סדר מיון</option>
            ...
            <h3><a style="background:url(images/more_icon.png) no-repeat right;" href="#">הגדרות הטופס</a></h3>
				<div>
                    <table class="leftstuff" dir="rtl" cellpadding="3" cellspacing="0" border="0" width="100%" id="Table1">
					<tr>
						<td width="20%" align="right"><p>דואל:</p><img src="images/ask22.png" id="Img3" original-title="דואל שישילחו פרטי הטופס לאחר מילוי" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
					  <textarea name="other1" cols="3" class="goodinputlong required" ><%If Editmode = "True" Then print objRs("other1") End If %></textarea>
					  </div>
					  </td>
					</tr>
					<tr>
						<td width="20%" align="right"><p>שם השדה של האימייל של המשתמש:</p><img src="images/ask22.png" id="Img4" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
					  <textarea name="other2" cols="3" class="goodinputlong" ><%If Editmode = "True" Then print objRs("other2") End If %></textarea>
					  </div>
					  </td>
					</tr>
					<tr>
						<td width="20%" align="right"><p>הודעה שתוצג למשתמש אחרי שליחה:</p><img src="images/ask22.png" id="Img5" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
					  <textarea name="other3" cols="3" class="goodinputlong" ><%If Editmode = "True" Then print objRs("other3") End If %></textarea>
					  </div>
					  </td>
					</tr>
					<tr>
						<td width="20%" align="right"><p>הפנייה אחרי מילוי:</p><img src="images/ask22.png" id="Img6" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
					  <textarea name="other4" cols="3" class="goodinputlong required" ><%If Editmode = "True" Then print objRs("other4") End If %></textarea>
					  </div>
					  </td>
					</tr>
					<tr>
						<td width="20%" align="right"><p>זמן להפנייה:</p><img src="images/ask22.png" id="Img7" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
					  <textarea name="other5"  class="goodinputshort required" ><%If Editmode = "True" Then print objRs("other5") End If %></textarea>
					  </div>
					  </td>
					</tr>
					<tr>
						<td width="20%" align="right"><p>סוג:</p><img src="images/ask22.png" id="Img2" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
                    <select name="other6">
                        <option value="1"<% If Editmode= "True" then %><% If objRs("other6") = 1 Then print " selected" %><% End if %>>טופס</option>
                        <option value="2"<% If Editmode= "True" then %><% If objRs("other6") = 2 Then print " selected" %><% End if %>>שאלון</option>
                    </select>
					  </div>
					  </td>
					</tr>
                    <tr>
						<td width="20%" align="right"><p>טקסט כפתור:</p><img src="images/ask22.png" id="Img8" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
						<td width="80%" align="right">
					<div style="position:relative;">
					  <textarea name="other7"  class="goodinputshort required" ><%If Editmode = "True" Then print objRs("other7") Else print "שלח" End If %></textarea>
					  </div>
					  </td>
					</tr>
            ...
            <div id="formsubmit">
            <input type="submit" name="save" style="margin:10px 0 0;" value="שמור" class="saveform"  />
           <input type="submit" name="update" style="margin:10px 0 0 10px;" value="עדכן" class="saveform"  />
            </div>
        </div>
