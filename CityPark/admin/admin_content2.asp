...
<script type="text/javascript">
	...
	function bringcontent(id,p,e) {
		...
		if (!$(e).hasClass("ajaxed")) {//first time open
			$("table#t"+id).load("admin_ajax.asp?ajax=c&id="+id+"&p="+p);
			$(e).addClass("ajaxed");
                   ...
                   <table class="leftstuff" dir="rtl" cellpadding="3" cellspacing="0" border="0" width="100%" id="Table3">
					...
					<td>
					  <select  class="goodselect validate" name="Categoryorder">
					    <option value="ID Desc"<%If Editmode = "True" Then %> <% If objRs("Categoryorder") = "ID Desc" Then Response.Write(" selected") %><%End If %>>אחרון שהוכנס מופיע ראשון</option>
					    <option value="ID ASC" <%If Editmode = "True" Then %><% If objRs("Categoryorder") = "ID ASC" Then Response.Write(" selected") %><%End If %>>ראשון שהוכנס מופיע ראשון</option>
