 ...
 %>   
    <div class="formtitle">
		...
		<div class="adminicons">
			<p><a href="admin_block.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></p>
			<p><a href="admin_block.asp?format=xls&<%Request.Querystring %>"><img src="images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p><a href="admin_block.asp?format=doc&<%Request.Querystring %>"><img src="images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p>
				<select name="records" onchange="location.href='admin_block.asp?records='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
...
If Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
	...
	If Request.QueryString("mode") = "search" then
		SQL = "SELECT * FROM Block WHERE SiteID=" & SiteID & " AND Name LIKE '%" & Request.form("search") & "%'"
    End If
...
Do While Not objRs.EOF And HowMany < objRs.PageSize
    ...
    <tr>
        ...
        <td><a href="admin_block.asp?ID=<% = objRs("Id") %>&action=edit"><img src="images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td><a href="admin_block.asp?ID=<% = objRs("Id") %>&action=copy"><img src="images/copy.gif" border="0" alt="שכפל " /></a></td>
        <td style="border-left:0px;"><a href="admin_block.asp?ID=<% = objRs("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק ');"><img src="images/delete.gif" border="0" alt="מחק" /></a></td>
	</tr>	
 <%	 HowMany = HowMany + 1
       objRs.MoveNext
		 Loop
	...
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
			        ...
			        Sql = "SELECT * FROM Block"
			        	Set objRs = OpenDB(sql)
				             ...
				             objRs("SiteID") = SiteID
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>בלוק נוסף בהצלחה. <a href='admin_Block.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' users='1; URL=admin_Block.asp?S=" & SiteID & "'>")
	       Case "edit"
	       	 Editmode= "True"

				    Sql = "SELECT * FROM Block WHERE ID=" & Request.QueryString("ID")
			            Set objRs = OpenDB(sql)
				                objRs("Name") = Trim(Request.Form("Name"))
								objRs("Text") = Request.Form("Text")
								objRs("image") = Request.Form("image")
          ...
          '//////CACHE//////////
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>בלוק נערך בהצלחה. <a href='admin_Block.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' users='1; URL=admin_Block.asp?S=" & SiteID & "'>")
          ...
          '//////CACHE//////////
	        ...
	        Case "delete"
				    Sql = "SELECT * FROM Block WHERE id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            Set objRs = OpenDB(sql)
			            objRs.Delete
			            objRs.Close
						    Response.Write("<br><br><p align='center'>בלוק נמחק בהצלחה. <a href='admin_Block.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
...
<form action="admin_block.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="block2" name="block2" class="_validate">
			<div class="incontentboxgrid2">
			    ...
			    <div class="rightform">
					...
					<tr>
						<td width="20%" align="right"><p>שם בלוק:</p><img src="images/ask22.png" id="east" original-title="שם הבלוק משמש להתמצאות בלבד" /></td>
					...
					<tr>
                        <td colspan="2" align="right">תוכן הבלוק:</td>
					</tr>
					<tr>
	...
	oFCKeditor.config("blockLangDirection")= "rtl"
	Else
	oFCKeditor.config("DefaultLanguage")= "en"
	oFCKeditor.config("blockLangDirection")= "ltr"
	End If
