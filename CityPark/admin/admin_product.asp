...
CheckSecuirty "Content"
 %>   
    <div class="formtitle">
        ...
        <form action="admin_product.asp?action=show&mode=search&records=<%=Request.Querystring("records") %>&category=<%=Request.form("category") %>&string=<%=Request.form("search") %>" method="post">
               <p>חיפוש:</p>
			   <select name="Active">
						<option value="1">פעיל</option>
						<option value="0">לא פעיל</option>
                </select>
					<% Set objRs = OpenDB("SELECT * FROM Content Where SiteID = " & SiteID & " AND Contenttype = 2")
                        If objRs.RecordCount > 0 then
						Set objRsCategory = OpenDB("SELECT * FROM Content Where SiteID = " & SiteID  & " AND Contenttype = 2")
		...
		<div class="adminicons">
			<p><a href="admin_product.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת מוצר" title="הוספת מוצר" /></a></p>
			<p><a href="admin_product.asp?format=xls&<%Request.Querystring %>"><img src="images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p><a href="admin_product.asp?format=doc&<%Request.Querystring %>"><img src="images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p>
				<select name="records" onchange="location.href='admin_product.asp?records='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
...
<thead>
    <tr>
		...
		<th style="width:8%;"><a href="admin_product_positions.asp?type=product&action=edit" class="_dialog { width: 500, title:'יש לגרור את המוצר לפי הסדר הרצוי',modal:true,position:top}">סדר דפים</a></th>
...
<%
        If Request.QueryString("lang") <> "" Then
               SQL = "SELECT [Content].*, Contentfather.FatherID, (SELECT count(*) as counter FROM Contentfather WHERE FatherID = [Content].id) AS COUNTER FROM [Content] INNER JOIN Contentfather ON [Content].Id = Contentfather.Contentid WHERE (Contentfather.FatherID = 0) AND Content.SiteID=" & SiteID & " AND  (Contenttype = 2)  AND (LangID LIKE '%" & Request.querystring("lang") & "%') ORDER By LangID ASC,ItemOrder ASC"
        Else
               SQL = "SELECT [Content].*, Contentfather.FatherID , (SELECT count(*) as counter FROM Contentfather WHERE FatherID = [Content].id) AS COUNTER FROM [Content] INNER JOIN Contentfather ON [Content].Id = Contentfather.Contentid WHERE (Contentfather.FatherID = 0) AND Content.SiteID=" & SiteID & " AND  (Contenttype = 2)  ORDER By LangID ASC,ItemOrder ASC"
        End If
...
%>
     <tr id="node-<%= objRsg("Id") %>" <% if id <> 0 Then %>class="child-of-node-<%= id %>"<% end if %> style="background-color: <% if color then %>#fff<% Else %>#f5f5f5<%End If %>;">
		...
		<td style="width:36%;text-align:right;" class="editlocalbig">	
			...
			<span onclick="bringproduct(<%= objRsg("Id")  %>,1,this)" style="float:right;cursor:pointer;width:5%;">
        ...
        <td  style="width:6%;"><% If objRsg("Active")= 1 Then %><img style="border:0px;" src="images/active.png" border="0" alt="" /><% Else %><img style="border:0px;" src="images/notactive.png" border="0" alt="" /><% End if %></td>
        
        <td style="border-left:0px;width:9%;"><%If objRsg("counter") = 0 Then %><a href="admin_product.asp?ID=<% = objRsg("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק דף זה');"><img src="images/delete.gif" border="0" alt="מחק" /></a>
		<% Else %>
...
Else
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
	        Case "add"
	   SqlCategory2 = "SELECT  TOP 1 * FROM Content WHERE SiteID= " & SiteID & " AND (Contenttype = 2) ORDER BY Id DESC"
			        ...
			        Sql = "SELECT * FROM Content WHERE SiteID= " & SiteID
			        	Set objRs = OpenDB(sql)
	                            ...
	                            For Each x In Request.Form
									If Mid(x, 1, Len("category_")) = "category_" Then
                                     If Request.Form(x) = "on" Then
                                        CheckCategorySecuirty(Request.Form(mid(x,16)))
                                     End If
						    		End If
								Next
				             objRs.Addnew
				             ...
				             objRs("urltext") = Trim(Request.Form("urltext"))
								...
								objRs("other1") = Request.Form("other1") 'Price
								objRs("other2") = Request.Form("other2") 'RetailPrice
								objRs("other3") = Request.Form("other3") 'CostPrice
								objRs("other4") = Request.Form("other4") 'Qty
								objRs("other5") = Request.Form("other5") 'Unit
								objRs("other6") = Request.Form("other6")
								objRs("other9") = Request.Form("other9") 'ship cost
								objRs("other10") = Request.Form("other10") 'ship cost
								objRs("other11") = Request.Form("other11") 'ship cost
								objRs("Couponcode") = Request.Form("Couponcode") 
								If Request.Form("Coupondiscount") = "" then
                                    Coupondiscount = 0
                                Else
                                    Coupondiscount = Request.Form("Coupondiscount")
                                End if
                                objRs("Coupondiscount") = Coupondiscount 'ship cost
						        ...
						        If LCase(Mid(r, 1, Len("other7"))) = "other7" Then
							        If Request.Form(r) = "on" Then
								        If c = "" OR c = NULL Then
									        c = Mid(r, Len("other7") + 1)
								        Else
									        c = c & "," & Mid(r, Len("other7") + 1)
								        End If							
							        End If			
						        End If
					        Next
					    objRs("other7") = c
                                For Each r in Request.Form
						        If LCase(Mid(r, 1, Len("other8"))) = "other8" Then
							        If Request.Form(r) = "on" Then
								        If d = "" OR c = NULL Then
									        d = Mid(r, Len("other8") + 1)
								        Else
									        d = d & "," & Mid(r, Len("other8") + 1)
								        End If							
							        End If			
						        End If
					        Next
						...
						objRs("menus") = c
                            M= ""
                                For Each r in Request.Form
						        If LCase(Mid(r, 1, Len("Mishloaj"))) = "mishloaj" Then
							        If Request.Form(r) = "on" Then
								        If M = "" OR M = NULL Then
									        M = Mid(r, Len("Mishloaj") + 1)
								        Else
									        M = M & "," & Mid(r, Len("Mishloaj") + 1)
								        End If							
							        End If			
						        End If
					        Next
		...
		If Request.Form("save") <> "" Then 'temporarysave
                        ...
                        xml.Send  
						Response.Write("<br><br><p align='center'>סטטוס עודכן בטוויטר. <a href='admin_product.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
                        Set xml = Nothing  
                     End If    
            Response.Redirect("admin_product.asp?notificate=הדף נוסף בהצלחה")
        Else
        	   Sqll = "SELECT  TOP 1 * FROM Content  ORDER BY Id DESC"
		    Set objRsSqll = OpenDB(Sqll)
            tSqll =objRsSqll(0)
            CloseDB(objRsSqll)
            Response.Redirect("admin_product.asp?ID="& tSqll &"&action=edit")
   		End If	
         '///////////Delete Cache/////////////
         ...
         '///////////Delete Cache/////////////		
							     ...
							     '   Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_product.asp?S=" & SiteID & "'>")
                        End If
	       Case "edit"
	       	 Editmode= "True"
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID= " & SiteID
			            Set objRs = OpenDB(sql)
                               ...
                               For Each x In Request.Form
								...
								objRs("other11") = Request.Form("other11") 'ship cost
								objRs("Couponcode") = Request.Form("Couponcode") 
								If request.Form("Couponcode") <> "" Then
                                     objRs("Coupondiscount") = Request.Form("Coupondiscount") 'ship cost
								End if
                                If Request.Form("Startdate") <> "" Then
                                    objRs("Startdate") = Request.Form("Startdate") 
                                End if
                                ...
                                If Request.Form("Enddate") <> "" Then
								    objRs("Enddate") = Request.Form("Enddate")& " 23:59:59"
                                End if
						...
						For Each r in Request.Form
						        If LCase(Mid(r, 1, Len("nilvim"))) = "nilvim" Then
							        If Request.Form(r) = "on" Then
								        If n = "" OR n = NULL Then
									        n = Mid(r, Len("nilvim") + 1)
								        Else
									        n =n & "," & Mid(r, Len("nilvim") + 1)
								        End If							
							        End If			
						        End If
					        Next
						...
						M= ""
						        ...
						        If LCase(Mid(s, 1, Len("Mishloaj"))) = "mishloaj" Then
							        If Request.Form(s) = "on" Then
								        If M = ""  Then
									        M = Mid(s, Len("Mishloaj") + 1)
								        Else
									        M = M & "," & Mid(s, Len("Mishloaj") + 1)
								        End If							
							        End If			
						        End If
					        Next
                        ...
                        xml.setRequestHeader "Content-Type", "content=text/html; charset=utf-8"  
                        xml.Send  
						Response.Redirect("admin_product.asp?notificate=הדף נשלח לטוויטר")
                        Set xml = Nothing  
                     End If
					 Response.Redirect("admin_product.asp?notificate=הדף נערך בהצלחה")
                    else
                        Response.Redirect("admin_product.asp?ID="&Request.QueryString("ID")&"&action=edit")
                    end if
         '///////////Delete Cache/////////////
                               deletecache objRs("Urltext")
                               Application.Contents.RemoveAll
	        ...
	        Case "copy"
			    ...
			    End If
					    ...
					    objRs("Menus") = c
						 ...
						 M= ""
                                For Each r in Request.Form
								print LCase(Mid(r, 1, Len("Mishloaj")))
						        If LCase(Mid(r, 1, Len("Mishloaj"))) = "mishloaj" Then
							        If Request.Form(r) = "on" Then
								        If M = "" OR M = NULL Then
									        M = Mid(r, Len("Mishloaj") + 1)
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            ...
			            objRs.Close
                         ExecuteRS("DELETE FROM [Contentfather] WHERE ContentID = " & Request.QueryString("ID"))
						    ...
						    Response.Redirect("admin_product.asp?notificate=הדף נמחק בהצלחה")
...
<form action="admin_product.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2" class="_validate">
			<div class="incontentboxgrid2">
			    ...
			    <div class="rightform">
					...
					<tr>
					  ...
					  <td align="right" id="loadcategories">טוען...
	...
	%>
				   ...
				   <% End If %>
					...
					<tr>
						<td align="left" colspan="2"><input type="submit" value="שמור" class="saveform" style="cursor:not-allowed;" disabled="disabled" /></td>
					</tr>
				</table>
				</div>
			</div>
           ...
           <h3><a style="background:url(images/banim.png) no-repeat right;" href="#">הגדרת בנים</a></h3>
				<div>
                   <table class="leftstuff" dir="rtl" cellpadding="3" cellspacing="0" border="0" width="100%" id="Table3">
                    ...
                    <tr>
                       <td align="right"><input id="Sonscategoryrecords" type="text" name="Sonscategoryrecords" <%If Editmode = "True" Then %> value="<% =objRs("Sonscategoryrecords")%>" <%Else%> value="3" <% End if %> class="goodinputshort" style="width:25px;" /></td>
                    </tr>
                    ...
                    <tr>
                       <td align="right"><input id="Text1" type="Sonsfirstrecords" name="Sonsfirstrecords" <%If Editmode = "True" Then %> value="<% =objRs("Sonsfirstrecords")%>" <%Else%> value="3" <% End if %> class="goodinputshort" style="width:25px;" /></td>
                    </tr>
                    <tr>
				...
				<div>
                    ...
                    dim fs
                        set fs=Server.CreateObject("Scripting.FileSystemObject")
                           othertemplate = templatelocation & "admin/admin_products/other.html"
                            if fs.FileExists(Server.MapPath(othertemplate))=true then
                           ' print SQL & "<br>" & Request.QueryString("mode")
                                 print getformlayout(GetUrl(othertemplate), SQL, "edit")
                            Else
                      %>
                       <tr>
                           <td align="right"><% = SiteTranslate("מחיר") %><img src="images/ask22.png" id="east13" original-title="יש לבחור את תבנית<br />תצוגת התוכן של הדף" /></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="other1" type="text" name="other1" <%If Editmode = "True" Then %> value="<% =objRs("other1")%>" <%Else%> value="0" <% End if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("מחיר מחירון") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="other2" type="text" name="other2" <%If Editmode = "True" Then %> value="<% =objRs("other2")%>" <%Else%> value="0" <% End if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("מחיר עלות") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="other3" type="text" name="other3" <%If Editmode = "True" Then %> value="<% =objRs("other3")%>" <%Else%> value="0" <% End if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("כמות במלאי") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="other4" type="text" name="other4" <%If Editmode = "True" Then %> value="<% =objRs("other4")%>" <%Else%> value="0" <% End if %> class="goodinputlong" /></td>
                         <tr>
                           <td align="right"><% = SiteTranslate("יחידת מידה") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="other5" type="text" name="other5" <%If Editmode = "True" Then %> value="<% =objRs("other5")%>" <%Else%> value="יח" <% End if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("קוד מוצר") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="other6" type="text" name="other6" <%If Editmode = "True" Then %> value="<% =objRs("other6")%>" <%Else%> value="00000" <% End if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("עלות משלוח") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="Text2" type="text" name="other9" <%If Editmode = "True" Then %> value="<% =objRs("other9")%>" <%Else%> value="00000" <% End if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("קוד קופון") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="Couponcode" style="width:100px" type="text" name="Couponcode" <%If Editmode = "True" Then %> value="<% =objRs("Couponcode")%>" <% end if %> class="goodinputlong" /></td>
                        </tr>
                        <tr>
                           <td align="right"><% = SiteTranslate("אחוז הנחה") %></td>
                        </tr>
                        <tr>
                         <td align="right"><input id="Coupondiscount" style="width:50px" type="text" name="Coupondiscount" <%If Editmode = "True" Then %> value="<% =objRs("Coupondiscount")%>"<% Else %> value="0" <% end if %>  class="goodinputlong" /></td>
                        </tr>

                        <% end if %>
                        <tr>
                           <td align="right"><% = SiteTranslate("אפשרויות") %></td>
                        </tr>
						<td align="right">
							<table class="pickmenu" id="pickermenu" cellpadding="0" cellspacing="0">טוען
						
						 </table>	
						 <script>
						 $("#pickermenu").load("admin_ajax.asp?ajax=3&objrs=<%=objRs("other7")%>&editmode=<%=Editmode%>",function() {sb=true;submitstate();});
						 </script>
						</td>
					  </tr>
                    </table>
                 </div>
            <h3><a style="background:url(images/more_icon.png) no-repeat right;" href="#">מוצרים קשורים</a></h3>
				<div>
                    <table cellpadding="0" cellspacing="0" border="0" dir="rtl" width="100%">
                     <tr>
					  <td align="right">
                      <div id="relateditems" Style="text-align:right;background:#FFFFFF;border:1px solid #CCCCCC;height:300px;overflow-y:auto;padding:5px;width:90%;">
            			
טוען...
</div><script>$("#relateditems").load("admin_ajax.asp?ajax=2&objrs=<%=objRs("other8")%>&editmode=<%=editmode%>",function() {sc=true;submitstate();});</script>
            ...
            <h3><a style="background:url(images/more_icon.png) no-repeat right;" href="#">מוצרים נילווים</a></h3>
				<div>
                    <table cellpadding="0" cellspacing="0" border="0" dir="rtl" width="100%">
                     <tr>
					  <td align="right">
                      <div id="nilvim" Style="text-align:right;background:#FFFFFF;border:1px solid #CCCCCC;height:300px;overflow-y:auto;padding:5px;width:90%;">
            			
טוען...
</div><script>$("#nilvim").load("admin_ajax.asp?ajax=7&objrs=<%=objRs("nilvim")%>&editmode=<%=editmode%>",function() {sd=true;submitstate();});</script>
				...
				<h3><a style="background:url(images/link_img.png) no-repeat right;" href="#">צורת משלוח לפי פריט</a></h3>
			        <div>
                    <table class="leftstuff" dir="rtl" cellpadding="3" cellspacing="0" border="0" width="100%" id="formleftfieldstable">
					<tr>			
						<td valign="top" width="40%" align="right">צורת משלוח:</td>
						<td align="right">
							<table class="pickmenu" cellpadding="0" cellspacing="0">
						  <%
							Set objRsMenuCategory2 = OpenDB("SELECT * FROM ShipingPerProduct Where SiteID= " & SiteID)			
							Do Until objRsMenuCategory2.Eof
								Selected = False
								If Editmode = "True" Then
									...
									For Each TheID In IDArray 							
										If Int(TheID) = objRsMenuCategory2(0) Then
											Selected = True
										End If
									Next
								End If
								...
								%>
								<tr>
									<td><input type="checkbox" name="Mishloaj<% = objRsMenuCategory2(0) %>"<% If Editmode = "True" AND Selected Then%> checked="1"<% End If %> /></td>
									<td><% = objRsMenuCategory2("Name") %></td>
								</tr>				
								<%
								objRsMenuCategory2.MoveNext
							Loop						
											
						  objRsMenuCategory2.Close
						  Set objRsMenuCategory2= Nothing
						  %>
