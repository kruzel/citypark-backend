 ...
 %>   
    <div class="formtitle">
		...
		<div class="adminicons">
			<p><a href="admin_form.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת כפתור חדש" title="הוספת כפתור חדש" /></a></p>
			<p><a href="admin_form.asp?format=xls&<%Request.Querystring %>"><img src="images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p><a href="admin_form.asp?format=doc&<%Request.Querystring %>"><img src="images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p>
...
<thead>
    <tr>
        <th>ID</th>
		<th style="width:40%;text-align:right;padding-right:40px;cursor:pointer;">שם הטופס</th>
	       	 ...
	       	 Editmode= "True"
				    ...
				    Sql = "SELECT * FROM Content WHERE Id=" & Request.QueryString("ID") & " AND SiteID= " & SiteID
			            Set objRs = OpenDB(sql)
				            ...
				            objRs("Name") = Request.Form("Menusname")
						 If  objRs("Contenttype")=3 OR Getconfig("AdminLevel") = 0 Then 
                            objRs("urltext") = Trim(Request.Form("urltext"))
                         End if
							objRs("LangID") = Request.Form("LangID")
...
<form action="admin_menu.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2" class="_validate">
			<div style="width:39%" class="incontentboxgrid2">
			    ...
			    <div class="rightform">
                    ...
                    <tr>
						...
						<% If  Request.QueryString("action") <> "edit" Or objRs("Contenttype")=3 OR Getconfig("AdminLevel") =0 Then %>
					...
					<tr>
						<td align="right"><p>דוא"ל מנהל:</p><img src="images/ask22.png" id="east2" original-title="תופיע בכתובת האתר אחרי הדומיין<br />מאוד חשוב לקידום בגוגל" /></td>
						<td align="right">
						<div style="position:relative;">
						<input class="goodinputshort required" type="text" name="email" <%If Request.QueryString("action") = "edit" Then %> value="<% =objRs("urltext")%>"  <% End if %>  />
						</div>
						</td>
					</tr>
						<% End If 
			...
			<div style="width:60%;margin:0;" id="left">
			<div id="formcontainer">
			<center>
				<div id="formcontent">
					<div id="formleftfields">
						<div class="formtitle2">
							<h1>הוספת שדות לטופס<span style="float:left;padding:0 0 0 10px;"><a href="#">הוסף שדה</a></span></h1>
							
						</div>
						<div class="rightform" style="padding:20px;text-align:right;border-bottom:1px solid #CCCCCC;border-left:1px solid #CCCCCC;border-right:1px solid #CCCCCC;-moz-border-radius:0 0 5px 5px;-webkit-border-radius:0 0 5px 5px;min-height:70px;">
							<table cellpadding="0" cellspacing="0" border="0" dir="rtl" width="100%">
								<tr>
									<td width="30%">שם השדה:</td>
									<td width="35%">סוג שדה:</td>
									<td width="30%">ערכים:</td>
									<td width="5%">חובה</td>
								</tr>
								<tr>
									<td><input type="text" class="goodinputshort required" /></td>
									<td>
										<select class="goodselect" style="margin:0;">
											<option>טקסט חופשי</option>
											<option>רשימה נפתחת</option>
											<option>בחירה</option>
										</select>
									</td>
									<td><input type="text" class="goodinputshort" /></td>
									<td><input type="checkbox" /></td>
								</tr>
							</table>
						</div>
						<div id="formsubmit">
						<input type="submit" style="margin:10px 0;" value="שמור" class="saveform"  />
						</div>
					</div>
