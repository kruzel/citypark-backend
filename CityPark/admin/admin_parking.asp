...
CheckSecuirty "Parking"
 ...
 format = Request.QueryString("format")
    ...
    Response.AddHeader "Content-Disposition", "filename=Parking.csv"
...
<thead>
    <tr>
		...
		<th style="width:8%;">תאריך הוספה</th>
		<th style="width:8%;">תאריך עריכה</th>
...
If  Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
    ...
    <tr style="background-color: <% if color then %>#fff<% Else %>#f5f5f5<%End If %>;">
        ...
        <td><% = objRsg("Createddate") %></td>
        <td><% = objRsg("Editdate") %></td>
			        	...
			        	Set objRs = OpenDB(sql)
				             objRs.Addnew
				              objRs("SiteID") = SiteID
	                           ...
	                           If Request.Form("payment")<> 1 Then
								        objRs("payment") = 0
								    Else
								        objRs("payment") = 1
								    End If
	                         '  objRs("price") = Request.Form("price")
	                          ...
	                          'Ran Brandes objRs("mekura") = Request.Form("mekura")
	                           ...
	                           objRs("UsersID") = Request.Form("UsersID")
...
<table class="addpark" style="text-align:right;" cellspacing="0" cellpadding="0" border="0" dir="rtl" width="100%">
	...
	<tr>
        ...
        <select id="UsersID" name="UsersID" >
        <%
		set objrUsers= OpenDB("SELECT * FROM Users Where SiteID=" & SiteID) %>
		<option value="null">בחר משתמש</option>
        <%do while not objrUsers.eof %>
				<option value="<%=objrUsers("Id")%>"<% if objrUsers("Id") = objRs("UsersID")  then %> selected="selected" <% End if %>><%=objrUsers("Name") & " " & objrUsers("Familyname") %></option>
        <%objrUsers.movenext
                loop %>
			</select>
            <% closeDB(objrUsers) %>
			<input id="vip" type="checkbox" value="1" name="vip"<% If Editmode = "True" AND  objRs("vip") = True then%> checked="checked" <% end if %>  /> VIP
...
<center>
    <div id="formcontent">
        <div id="formleftfields">
            ...
            <div id="formsubmit">
            <input type="submit" style="margin:10px 0 0;cursor:not-allowed;" value="שמור" id="submi" class="saveform" disabled="disabled" />
            </div>
        </div>
