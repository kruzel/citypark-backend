...
CheckSecuirty "Shipping"
 %>   
    <div class="formtitle">
        <h1>ניהול משלוח לפי פריט</h1>
		<div class="admintoolber">
        <form action="admin_productshiping.asp?action=show&mode=search" method="post">
               <p>חיפוש:</p>
		...
		<div class="adminicons">
			<p><a href="admin_productshiping.asp?action=add"><img src="images/addnew.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></p>
			<p><a href="admin_productshiping.asp?format=xls&<%Request.Querystring %>"><img src="images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p><a href="admin_productshiping.asp?format=doc&<%Request.Querystring %>"><img src="images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></p>
			<p>
...
<thead>
    <tr>
		...
		<th style="width:10%;text-align:center;padding:0 10px 0 0">מחיר</th>
        <th>ערוך</th>
        <th>שכפל</th>
        <th>מחק</th>
...
If Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
		SQL = "SELECT * FROM ShipingPerProduct WHERE SiteID=" & SiteID
	If Request.QueryString("mode") = "search" then
		SQL = "SELECT * FROM ShipingPerProduct WHERE SiteID=" & SiteID & " AND Name LIKE '%" & Request.form("search") & "%'"
    End If
...
Do While Not objRs.EOF And HowMany < objRs.PageSize
    ...
    <tr>
		...
		<td style="text-align:center;"><% = objRs("alut") %></td>
        <td><a href="admin_productshiping.asp?ID=<% = objRs("Id") %>&action=edit"><img src="images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td><a href="admin_productshiping.asp?ID=<% = objRs("Id") %>&action=copy"><img src="images/copy.gif" border="0" alt="שכפל " /></a></td>
        <td style="border-left:0px;"><a href="admin_productshiping.asp?ID=<% = objRs("Id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק ');"><img src="images/delete.gif" border="0" alt="מחק" /></a></td>
	</tr>	
 <%	 HowMany = HowMany + 1
       objRs.MoveNext
		 Loop
...
Else
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
	        Case "add"
	        Editmode= "False"
			      Set objRsUrl = OpenDB("SELECT * FROM ShipingPerProduct WHERE SiteID=" & SiteID)
			        ...
			        Sql = "SELECT * FROM Shipping"
			        	Set objRs = OpenDB(sql)
				             ...
				             objRs("SiteID") = SiteID
								...
								objRs("alut") = Request.Form("alut")
								objRs.Update
								objRs.Close
									Response.Redirect("admin_productshiping.asp?notificate=משלוח נוסף בהצלחה")
	       Case "edit"
	       	 Editmode= "True"

				    Sql = "SELECT * FROM ShipingPerProduct WHERE Id=" & Request.QueryString("ID")
			            Set objRs = OpenDB(sql)
				                objRs("Name") = Trim(Request.Form("Name"))
								objRs("alut") = Request.Form("alut")
								objRs.Update
								objRs.Close
									Response.Redirect("admin_productshiping.asp?notificate=משלוח נערך בהצלחה")
	        ...
	        Case "delete"
				    Sql = "SELECT * FROM ShipingPerProduct WHERE Id=" & Request.QueryString("ID") & " AND SiteID=" & SiteID
			            Set objRs = OpenDB(sql)
			            objRs.Delete
			            objRs.Close
									Response.Redirect("admin_productshiping.asp?notificate=מאפיין נמחק בהצלחה")
	            End Select
		Else
				SQL = "SELECT * FROM ShipingPerProduct WHERE Id=" & Request.QueryString("ID") & " And SiteID=" & SiteID
...
<form action="admin_productshiping.asp?mode=doit<% If Editmode="True" Then %>&ID=<% = objRs("Id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="block2" name="block2" class="_validate">
			<div class="incontentboxgrid2">
			    ...
			    <div class="rightform">
                    ...
                    <td align="right" valign="top"><input  type="text"  name="alut" cols="40"<%If Editmode = "True"  Then%> value="<%= objRs("alut")%>"<% End if %> /></td>
                    </tr>
