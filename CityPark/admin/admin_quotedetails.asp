...
<% 
End If
CheckSecuirty "DshopQuoteDetails"
...
If Request.QueryString("p")= "" AND Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
		SQL = "SELECT * FROM DshopQuoteDetails WHERE SiteID=" & SiteID & " ORDER BY id DESC"
...
<center>
    <div style="width:99%;height:43px;background:#fff;line-height:43px;border:1px solid #ccc;">
        <div style="width:300px;float:right;line-height:43px;text-align:right;padding-right:10px;color:#25BAE2;font-size:22px;">ניהול הצעות מחיר</div>
        <div style="width:40px;float:left;line-height:43px;"><a href="admin_quotedetails.asp?action=add"><img src="../admin/images/add.gif" border="0" alt="הוספת דף חדש" title="הוספת דף חדש" /></a></div>
        <div style="width:40px;float:left;line-height:43px;"><a href="admin_quotedetails.asp?format=xls&<%Request.Querystring %>"><img src="../admin/images/excel.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></div>
        <div style="width:40px;float:left;line-height:43px;"><a href="admin_quotedetails.asp?format=doc&<%Request.Querystring %>"><img src="../admin/images/word.gif" border="0" alt="יצוא לאקסל" title="יצוא לאקסל" /></a></div>
        <div style="float:left;line-height:43px;margin-left:10px;width:50px;">
        <select style="float:right;font-family:arial;height:23px;margin-top:9px;width:50px;" name="records" onchange="location.href='admin_quotedetails.asp?records='+escape(this.options[this.selectedIndex].value)+'&SiteID=<%=SiteID%>';">
        ...
        <form action="admin_quotedetails.asp?action=show&mode=search" method="post">
		            ...
		            <td style="border-bottom:1px solid #ccc;height:25px;vertical-align:middle;" align="center" colspan="8">
	      	            <table width="200" cellspacing="0" align="center">
	                        ...
	                        <% If objRs.AbsolutePage > 1 Then %>
					                        <a href="admin_quotedetails.asp?page=<% = objRs.AbsolutePage - 1 %>" style="text-decoration: none"><% End If%>
...
<tbody>
    ...
    <tr onmouseover="Mark(this);" onmouseout="Unmark(this);">
        ...
        <%if format = "" then %>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_quotedetails.asp?ID=<% = objRs("id") %>&action=edit"><img src="/sites/cms/layout/images/edit.gif" border="0" alt="ערוך" /></a></td>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_quotedetails.asp?ID=<% = objRs("id") %>&action=copy"><img src="/sites/cms/layout/images/copy.gif" border="0" alt="שכפל דף" /></a></td>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"></td>
        <td style="border-left:1px solid #eee;border-bottom:1px solid #eee;line-height:25px;" align="center"></td>
        <td style="border-bottom:1px solid #eee;line-height:25px;" align="center"><a href="admin_quotedetails.asp?ID=<% = objRs("id") %>&mode=doit&action=delete" onclick="return confirm('?האם אתה בטוח שברצונך למחוק דף זה');"><img src="/sites/cms/layout/images/delete.gif" border="0" alt="מחק" /></a></td>
        <% End If ' format %>
    </tr>	
    			<% HowMany = HowMany + 1
				objRs.MoveNext
				            ...
				            %>
					            <tr>
							            ...
							            <td width="177"><font size="2" face="Arial">
							                <div align="center">
	                        <% If objRs.AbsolutePage > 1 Then %>
					                        <a href="admin_quotedetails.asp?page=<% = objRs.AbsolutePage - 1 %>&records=<% = records %>" style="text-decoration: none"><% End If%>
					                        <font color="#808080">דף קודם</font></a>
					                    </td>
					                        <td width="25">
...
Else
	If Request.QueryString("mode") = "doit" Then
	    Select Case Request.QueryString("action")
	        Case "add"
	        Editmode= "False"
			        Sql = "SELECT * FROM DshopQuoteDetails"
			        	Set objRs = OpenDB(sql)
				             ...
				             objRs("SiteID") = SiteID
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>מתעניין נוסף בהצלחה. <a href='admin_quotedetails.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_quotedetails.asp?S=" & SiteID & "'>")
	       Case "edit"
	       	 Editmode= "True"

				    Sql = "SELECT * FROM DshopQuoteDetails WHERE id=" & Request.QueryString("ID")
			            Set objRs = OpenDB(sql)
				                objRs("b_name") = Request.Form("b_name")
				                objRs("contact_person_name") = Request.Form("contact_person_name")
				                objRs("phone") = Request.Form("phone")
								...
								objRs.Update
								objRs.Close
									Response.Write("<br><br><p align='center'>מתעניין נערך בהצלחה. <a href='admin_quotedetails.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							        Response.Write("<meta http-equiv='Refresh' content='1; URL=admin_quotedetails.asp?S=" & SiteID & "'>")
	        ...
	        Case "delete"
				    Sql = "SELECT * FROM DshopQuoteDetails WHERE id=" & Request.QueryString("ID")
			            ...
			            objRs.Delete
			            objRs.Close
						    Response.Write("<br><br><p align='center'>מתעניין נמחק בהצלחה. <a href='admin_quotedetails.asp?S=" & SiteID & "'>לחץ להמשך</a>!</p>")
							Response.Write("<meta http-equiv='Refresh' content='0; URL=admin_quotedetails.asp?S=" & SiteID & "'>")
	        
	        Case "positions"
	                    Response.Write("<br><br><p align='center'>מיקום  שונה בהצלחה. <a href='admin_quotedetails.asp'>לחץ להמשך</a>!</p>")
			            Response.Write("<meta http-equiv='Refresh' content='2; URL=admin_quotedetails.asp'>")

	            End Select
		Else
				SQL = "SELECT * FROM DshopQuoteDetails WHERE id=" & Request.QueryString("ID") & " And SiteID=" & SiteID
			If Request.QueryString("action") = "add" Then
			 SQL = "SELECT * FROM DshopQuoteDetails WHERE SiteID=" & SiteID   
			End If	
				Set objRs = OpenDB(SQL)
 ...
 %>
<form action="admin_quotedetails.asp?mode=doit<% If Editmode Then %>&ID=<% = objRs("id")%><% ENd IF %>&action=<%=Request.Querystring("action") %>" method="post" id="content2" name="content2">
