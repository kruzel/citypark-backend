...
Set MyFolder=MyFileObject.GetFolder(phisicalpath &  Application(ScriptName & "Sitename") & Replace(Request.QueryString("subfolder"), "/", "\"))
Set fold=MyFileObject.GetFolder(phisicalpath &  Application(ScriptName & "Sitename") & "\Content")
...
<script language="javascript">
	...
	{
		opener.<% = Request.QueryString("object") %>.innerText= "/"+v; 
		window.close();
	}
</script>
...
<%
Set fs = CreateObject("Scripting.FileSystemObject")
a = fs.GetFolder(phisicalpath &  Application(ScriptName & "Sitename") & "/Content/")
		...
		<td>
			<table align=center>
				<tr>
					...
					%>
						  <td>
					<table align=center height="230" name="ta<% = Replace(f2.name, ".", "") %>" id="ta<% = Replace(f2.name, ".", "") %>" onmouseout="javascript:ChangeBorderColor(this.id);" onmouseover="javascript:ChangeBorderColor(this.id);" border="2" bgcolor="#FFFF99" bordercolor="#000000" width="100" cellspacing="5">
						<tr>
							<td colspan="3" align="center">
								...
								Select Case lcase(sf(1))
									...
									Case "jpg"
										...
										B = Replace(B, "/", "\")
										%>
											<img border="1" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="../resize.asp?path=<% = B %>&width=100" <% If Request.QueryString("object") <> "" Then %>ondblclick="javascript:SetImagePath('<% = Request.QueryString("object") %>','<% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" & f2.name %>')"<% End If %> onclick="javascript:SetInfo(<% = f2.Size %>,'<% = f2.Type %>')" ></td>
									<% Case "gif" %>
											<img border="1" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="<% = GetURL(f2.name) %>" <% If Request.QueryString("object") <> "" Then %>ondblclick="javascript:SetImagePath('<% = Request.QueryString("object") %>','<% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" & f2.name %>')"<% End If %> onclick="javascript:SetInfo(<% = f2.Size %>,'<% = f2.Type %>')" ></td>
									
									<% Case Else %>
											<img border="1" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="images/icons/<% = sf(1)%>.gif" <% If Request.QueryString("object") <> "" Then %>ondblclick="javascript:SetImagePath('<% = Request.QueryString("object") %>','<% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" & f2.name %>')"<% End If %> onclick="javascript:SetInfo(<% = f2.Size %>,'<% = f2.Type %>')" ></td>
								
								<% End Select %>
							<td>
						</tr>
						<tr>
							<td width=95% colspan="3"><% = f2.name %></td>
						</tr>
						<tr>
							<td colspan="3">
							<% 
								Response.Write(ConvertBytes(f2.size))
							%>
						...
						<tr>
							<td width="33"><a href="default.asp?<% = Request.QueryString %>&m=delete&filename=<% = f2.name %>">מ</a></td>
							<td width="33">ש</td>
								<td width="33"><% =  Application(ScriptName & "UploadPath") %><a href="<% = "..\" &  Application(ScriptName & "UploadPath") & "\" & Request.QueryString("subfolder") & "\" & f2.name %>" rel="lightbox">ה</a></td>
						</tr>
					</table>	  
					<% 
					I = I + 1
