...
<script type="text/javascript" src="../../filemanger/js/prototype.js"></script>
<script type="text/javascript" src="../../filemanger/js/scriptaculous.js?load=effects"></script>
<script type="text/javascript" src="../../filemanger/js/lightbox.js"></script>

<head>

<meta http-equiv="Content-Type" content="text/html; charset=windows-1255">
<link rel="stylesheet" href="fm.css" type="text/css" media="screen" />
...
AllowedExt = "jpg,gif,png,doc,xls,csv,pdf" 
AllowedExt = Split(AllowedExt, ",")
...
Else
  ...
  If File.ImageType <> "UNKNOWN" Then
	...
	If NOT LCASE(Request.QueryString("resize")) = "false" Then
   	If NOT jpeg.OriginalWidth <= 600 Then
	   	X = jpeg.OriginalWidth / 600
...
MainFolderPath = Server.MapPath("\") & "/sites/" & Application(ScriptName & "ScriptPath")
...
Function GetURL(FileName)
	...
	If lcase(splitted(1)) = "jpg" Then
		GetURL = "/resize.asp?path=" & Server.MapPath("../../sites/" & Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" & f2.name) & "&width=100&height=100" 
	Else
		GetURL = "../../sites/" & Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" & f2.name
	End If
...
<script language="javascript">
	...
	if (answer){
		window.opener.document.getElementById('<% = Request.QueryString("object") %>').value = <% If Request.QueryString("Get") <> "FileNameOnly" Then %>"../" + <% End If %>v; 
		window.close();
	}
	}
</script>

<table align="center" width="585" bgcolor=#FFFFFF>
	<tr>
	...
	<tr><td width="0" rowspan="2">&nbsp;</td>
		<td valign="top" width="203" rowspan="2" >
...
Sub LFolder(MainPath)
	...
	End IF
	If Allow = True Then
	Response.Write("<ul><li>" & FolderName & " <small>" & ConvertBytes(Folder.Size) & "</small></ul><BR>")
	...
	If	SubFolder.Name <> "_vti_cnf" then

			Response.Write("<a href=""default.asp?Get=" & Request.QueryString("Get") & "&object=" & Request.QueryString("object") & "&subfolder=" & Link & """>")
...
%>
		...
		<td width="341" valign="top" bgcolor="#C0C0C0">
			<p style="text-align: center">רשימת קבצים</td>
	</tr>
	<tr>
		
		<td width="341" valign="top" colspan="3">
			<table align=left width="121">
				<tr>
					<% 
					...
					%>
						  <td>
					<div align="center">
					<table name="ta<% = Replace(f2.name, ".", "") %>" id="ta<% = Replace(f2.name, ".", "") %>" onmouseout="javascript:ChangeBorderColor(this.id);" onmouseover="javascript:ChangeBorderColor(this.id);" border="2" bordercolor="#000000" width="80" cellspacing="1">
						<tr>
							<td colspan="2" align="center" style="float:left;">
								<%
								sf = Split(f2.name, ".")
								...
								Select Case lcase(sf(1))
									...
									Case "jpg"
										...
										B = B & "\" & f2.name 
										B = Replace(B, "/", "\")
										%><img border="1" alt="<% = f2.name%>" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="../../resize.asp?path=<% = B %>&width=80"  width="80" <% If Request.QueryString("object") <> "" Then %> onclick="SetImagePath('<% = Request.QueryString("object") %>','<% If Request.QueryString("Get") <> "FileNameOnly" Then %><% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" %><% End If %><% = f2.name %>')"<% End If %> height="64" ></td>
									<% Case "gif" %>
											<img border="1" alt="<% = f2.name%>" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="<% = GetURL(f2.name) %>" <% If Request.QueryString("object") <> "" Then %> width="100" onclick="SetImagePath('<% = Request.QueryString("object") %>','<% If Request.QueryString("Get") <> "FileNameOnly" Then %><% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" %><% End If %><% = f2.name %>')"<% End If %>  ></td>
									<% Case "png" %>
											<img border="1" alt="<% = f2.name%>" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="<% = GetURL(f2.name) %>" <% If Request.QueryString("object") <> "" Then %> width="100" onclick="SetImagePath('<% = Request.QueryString("object") %>','<% If Request.QueryString("Get") <> "FileNameOnly" Then %><% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" %><% End If %><% = f2.name %>')"<% End If %>  ></td>
									
									<% Case Else %>
											<img border="1" alt="<% = f2.name%>" name="<% = Replace(f2.name, ".", "") %>" id="<% = Replace(f2.name, ".", "") %>" src="images/icons/<% = sf(1)%>.gif" <% If Request.QueryString("object") <> "" Then %> width="100" onclick="SetImagePath('<% = Request.QueryString("object") %>','<% = Application(ScriptName & "ScriptPath") & Request.QueryString("subfolder") & "/" & f2.name %>')"<% End If %> ></td>
						...
						<a onclick = "if (! confirm('?למחוק')) { return false; }" href="default.asp?<% = Request.QueryString %>&m=delete&filename=<% = f2.name %>">
					<img src="images/remove.png" ></a>
						<span  onclick="javascript:SetInfo(<% = f2.Size %>,'<% = f2.Type %>')">
						<img src="images/info.png" >&nbsp;</span></td>
						</tr>
					</table>	  
					</div>
					<% 
