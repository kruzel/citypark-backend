...
<script type='text/javascript'>
    $(function () {
		...
		$("#categorylist").load("admin_ajaxusers.asp?ajax=4&id=<%=Request.QueryString("id")%>&action=<%=Request.QueryString("action")%>",function() {sb=true;sc=true;sa = true;initTree();});
...
CheckSecuirty "Users"
	             ...
	             '   myMail.From = "mailer@outmail.info" 'Trim(Request.Form("Sender"))
	                myMail.From = Trim(Request.Form("Sender"))
			myMail.Configuration.Fields.Item("urn:schemas:mailheader:x-content-type") = "text/html; charset=utf-8"
			myMail.Configuration.Fields.Item("http://schemas.microsoft.com/cdo/configuration/sendusing") = 2
			myMail.Configuration.Fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpserver") = smtp
			myMail.Configuration.Fields.Item("http://schemas.microsoft.com/cdo/configuration/smtpauthenticate") = 1
	                ...
	                Length = 0
		                ...
		                For Each x In Request.Form
		                    If Mid(x, 1, 5) = "Send_" Then	
			                    If Request.Form(x) = "on" Then	
				                    If Bcc = "" Then
					                    Bcc = Lcase(Request.Form("OSend_" & Mid(x, 6)))
					                    theemail = Lcase(Request.Form("OSend_" & Mid(x, 6)))
					                    theid = Lcase(Request.Form("OSeID_" & Mid(x, 6)))
				                    Else
					                    Bcc = Bcc & ", " & Lcase(Request.Form("OSend_" & Mid(x, 6)))
				                    End If	
					            
					            If Request.Form("Template")<> "" Then
                                ssss="Select * From [Users] Where MailingList=1 AND email='" & Request.Form("OSend_" & Mid(x,6)) & "' AND SiteID = " & SiteID
	                           '  print ssss 
                                    Set Rs = OpenDB(ssss)
                                        myMail.HtmlBody = GetPostTemplate(GetRecordSetTemplate(GetUrl(Request.Form("Template")),Rs))
				        ...
				        If nRun = EmailCount Or Length = nRun2 Then
					        nRun = 1
					        myMail.Bcc = Bcc
		                    myMail.HtmlBody = myMail.HtmlBody & "<div style=""direction:rtl;text-align:right;"">דבר דאר זה נשלח לרשימת תפוצה של האתר.<br />להסרה מרשימת התפוצה לחץ <a href=http://" & LCase(Request.ServerVariables("HTTP_HOST"))& "/mailinglist.asp?mode=Del&email=" & theemail & "&ID=" & theid & ">כאן</a>.</div><br />"
							print "נשלח ל-" & Bcc & "<br>"
							myMail.Send
							If Err.Number<>0 Then
			'SendEmail=Err.Description
							print "נשלח ל-" & Email & "<br>" & Err.Description & "<br>"
							Err.Clear
						End If
...
sendmode = True 
 %>
<form action="admin_users.asp?type=sendmail" method="post" >
<div class="rightform">
	<table style="padding:0 20px;">
	    <tr>
	       <th align="right" width="10%">השולח</th>
	       <td width="40%"><input class="goodinputlong" type=text name="Sender" value="newsletter<% = Replace(LCase(Request.ServerVariables("HTTP_HOST")),"www.","@") %>"></td>
	    ...
	    <tr>
            ...
            <td width="40%" align="right">
				...
				<input type="button" class="goodinputbrows" value="חפש בשרת" onclick="BrowseFile('Template');"/>
			</td>
        ...
        <tr>
			<td align="center" valign="top" align="left" colspan="4">
			 <% 
                           ...
                           newlettertemplate = templatelocation & "newsletter/newsletter.html"
                            if fs.FileExists(Server.MapPath(newlettertemplate))=true then
                              template =  GetUrl(newlettertemplate)
                            end if
...
If Request.QueryString("ID")= "" AND Request.QueryString("action")<> "add" Then
    ...
    End If
            ...
            Do While Not objRsg.EOF And HowMany < objRsg.PageSize
            	%>
    <tr>
          <input type="hidden" name="OSend_<% = objRsg(0) %>" value="<% = objRsg("Email") %>" />
          <input type="hidden" name="OSelular_<% = objRsg(0) %>" value="<% = objRsg("cellular") %>" />
          <input type="hidden" name="OSeID_<% = objRsg(0) %>" value="<% = objRsg(0) %>" />
				...
				End if		
								...
								If Request.form("sendmailtocustomer") = "on" then
											If Getconfig("sender") = "" then
												sender = Sender
											Else
												sender = Getconfig("sender")
											End if
								SendMailForm "אושרה כניסתך למערכת",sender,objRs("Email"),Templatelocation & "customercomfirmation.html"
								'response.end
								End If
            ...
            <h3><a style="background:url(images/more_icon.png) no-repeat right;" href="#">שדות נוספים</a></h3>
				<div>
                    <table class="leftstuff" dir="rtl" cellpadding="3" cellspacing="0" border="0" width="100%" id="Table1">
                       <%
                        set fs=Server.CreateObject("Scripting.FileSystemObject")
                           othertemplate = templatelocation & "admin/admin_users/other.html"
